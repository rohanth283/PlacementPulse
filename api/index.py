...
# Import our modular dependencies
from api.database import get_db_connection, init_db
from api.auth import hash_password, get_current_user_id, check_rate_limit
from api.ai import perform_hybrid_search, generate_content_with_retry
...
app = FastAPI(title="PlacementPulse API")
...
# ----------------------------------------------------
# Experiences Query Routes
# ----------------------------------------------------
@app.get("/api/companies")
def get_companies(user_id: int = Depends(get_current_user_id)):
    from api.ai import documents, load_documents_index
    if not documents:
